...
const OWNER_TYPES = [
  { value: 'system', label: 'Маркетплейс' },
    ...
    <div className="page-grid">
      ...
      <section className="panel-card account-controls">
        <label className="field">
          <span>Владелец</span>
          <select className="field-control" value={filters.ownerType} onChange={(event) => onFilterChange('ownerType', event.target.value)}>
        ...
        <label className="field">
          ...
          <input
            ...
            disabled={filters.ownerType === 'system'}
            placeholder="Например, 12"
          />
        </label>
        <label className="field">
          <span>Валюта</span>
          <select className="field-control" value={filters.currencyCode} onChange={(event) => onFilterChange('currencyCode', event.target.value)}>
      ...
      <section className="account-card-grid">
        {accounts.length === 0 ? (
          <div className="empty-panel">Счета пустые или еще не созданы.</div>
        ) : (
...
function formatMoney(amount, currencyCode) {
  return `${toText(amount) || '0'} ${formatCurrency(currencyCode)}`
}
...
function buildTransactionView(transaction) {
  ...
  return {
    ...
    asset: amounts.map((amount) => amount.currency).join(' / ') || 'Операция',
    amounts,
